...
// It will bring CurrentAxpDriverInterface into scope.
use crate::{
    ...
    I2CBus: embedded_hal::i2c::I2c<Error = E>,
{
    ...
    fn write_register(
        ...
        if data.len() > 2 {
            // AXP192 registers are typically 1 byte. Some writes might involve 2 bytes for ADC settings.
            return Err(AxpError::NotImplemented(
                "Write data potentially too large for typical AXP192 registers, check datasheet.",
            ));
        }
